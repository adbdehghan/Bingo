...
extension PageView {
  ...
  class func pageViewOnView(_ view: UIView, itemsCount: Int, bottomConstant: CGFloat, radius: CGFloat, selectedRadius: CGFloat, itemColor: @escaping (Int) -> UIColor) -> PageView {
    ...
    // add constraints 
    for (attribute, const):(NSLayoutAttribute,CGFloat) in [(NSLayoutAttribute.left, 0) as (NSLayoutAttribute,CGFloat), (NSLayoutAttribute.right, 0) as (NSLayoutAttribute,CGFloat), (NSLayoutAttribute.bottom, bottomConstant) as (NSLayoutAttribute,CGFloat)]{
      (view, pageView) >>>- {
